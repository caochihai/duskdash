...
## 8. API contract đề xuất cho backend

```
GET    /conversations                      -> { data: Conversation[] }
POST   /conversations                      -> { data: Conversation }
PATCH  /conversations/:id                  -> { data: Conversation }   // { title?, pinned? }
DELETE /conversations/:id                  -> 204
GET    /conversations/:id/messages         -> { data: ChatMessage[] }
POST   /conversations/:id/messages         -> { message: ChatMessage } // { content, attachmentIds?, mode? }
POST   /conversations/:id/messages/stream  -> NDJSON stream of StreamEvent
POST   /attachments                        -> { attachment: ChatAttachment }  // multipart
```

`StreamEvent` (một JSON mỗi dòng — hợp với SSE / WebSocket / chunked HTTP):

```ts
| { type: 'phase';    phase: AIProcessingPhase }
| { type: 'delta';    text: string }
| { type: 'complete'; message: ChatMessage }
| { type: 'error';    error: ApiError }
```

Kiểu dữ liệu đầy đủ: `src/types/`.
