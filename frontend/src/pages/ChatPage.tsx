...
import { CustomerWorkspacePanel } from '@/components/ai/CustomerWorkspacePanel';
import {
  DEMO_APPROVAL_LIMIT,
  MOCK_CUSTOMERS,
  findLoansByCustomer,
...
export default function ChatPage() {
  ...
  const handleSend = useCallback(
    async (content: string, attachments: ChatAttachment[]) => {
      ...
      setComposerValue('');
      await send({ content, attachments, mode });
    },
    [activeConversationId, createConversation, messageApi, mode, send],
  );
  ...
  const handleViewCustomer = useCallback((customerId: string) => {
    const found = MOCK_CUSTOMERS.find((item) => item.id === customerId);
    if (found) {
  ...
  const customerRecords = useMemo(
    () => (activeCustomer ? findLoansByCustomer(activeCustomer.id) : []),
    [activeCustomer],
  );
