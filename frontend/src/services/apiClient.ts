...
/**
 * Nơi lấy access token trong tương lai.
 * Hiện tại demo chưa có auth nên trả về null.
 */
...
export function normalizeError(error: unknown): ApiError {
  ...
  if (axios.isAxiosError(error)) {
    ...
    return {
      ...
      message: body?.message ?? USER_MESSAGE[code] ?? USER_MESSAGE[API_ERROR_CODES.unknown],
      status,
      details: body?.errors,
    };
  }
